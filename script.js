...
function cheking() {
  ...
  function cantblank (fieldId, errId){
    ...
    { document.getElementById(errId).innerHTML ="This field can't be blank!";
  ...
  function numbers (fieldId, errId) {
    ...
    st = st.replace(/[" "]/gi,""); 
    if (isNaN(st)) {
      document.getElementById(errId).innerHTML= "The value can only be a number";
      document.getElementById(fieldId).style.borderColor=("var(--clr-Red)");
      ok = false;
    }
  }
  ...
  if (document.getElementById("err-number").innerHTML.length==0) {
    ...
    if( st.length != 16)
     {
      document.getElementById("err-number").innerHTML= "The card number must be 16 character long!";
    ...
    if (document.getElementById("err-date").innerHTML.length==0) {
      if (Number(document.getElementById("exp-date").value) >12) {
        document.getElementById("err-date").innerHTML ="The month can't be bigger then 12!"
